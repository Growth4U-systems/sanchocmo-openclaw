...
in its **Runtime engine** step. Pick `hermes` or `external-http`:

- **`hermes`** then asks for a **mode** (`bridge` / `external` / `command`):
  - **`bridge`** (default) — Hermes runs *embedded inside* the Sancho container.
    No gateway URL to enter (it's derived to `http://127.0.0.1:18795`), the chat
    secret is autogenerated, and `HERMES_CLI_PROVIDER` is derived from the model
    provider you chose. The simplest path on a single machine. The default flips
    to `external` if a `HERMES_GATEWAY_URL` is already in the environment.
  - **`external`** — point Sancho at a Hermes gateway you run yourself: enter the
    URL + secret; the wizard runs a reachability healthcheck. The URL is called
    from *inside* the container, so a gateway on the same host is
    `http://host.docker.internal:<port>`, **not** `localhost`.
  - **`command`** — the wizard stores a `HERMES_GATEWAY_COMMAND` that boots the
    gateway when the container starts.
- **`external-http`** asks for the gateway URL + secret + protocol + health path
  and runs the same reachability healthcheck.

The wizard only asks for the OpenClaw gateway/legacy host ports when the runtime
is `openclaw` (other runtimes never bind them). Alternatively — or for
scripted/quick installs — pass the runtime env vars before running the installer
or wizard:
