...
class WizardTestCase(unittest.TestCase):
    ...
    # --- hermes modes (SAN-486) ---------------------------------------------

    def test_hermes_bridge_mode_derives_and_generates(self):
        r = self.run_wizard(
            {"SANCHO_RUNTIME": "hermes", "HERMES_MODE": "bridge",
             "PROVIDER": "anthropic", "ANTHROPIC_API_KEY": "sk-ant-xyz"},
            args=("--advanced",),
        )
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(r.env.get("SANCHO_RUNTIME"), "hermes")
        self.assertEqual(r.env.get("HERMES_BRIDGE_ENABLED"), "1")
        # gateway URL derived to the in-container bridge; never asked
        self.assertEqual(r.env.get("HERMES_GATEWAY_URL"), "http://127.0.0.1:18795")
        # chat secret autogenerated (64 hex chars), not left blank
        self.assertRegex(r.env.get("HERMES_CHAT_SECRET", ""), r"^[0-9a-f]{64}$")
        # CLI provider derived from the model provider chosen in the wizard
        self.assertEqual(r.env.get("HERMES_CLI_PROVIDER"), "anthropic")

    def test_hermes_bridge_is_default_without_url(self):
        # No HERMES_MODE and no gateway URL → bridge (no URL required to proceed).
        r = self.run_wizard(
    ...
    def test_hermes_external_mode_requires_url(self):
        r = self.run_wizard(
            {"SANCHO_RUNTIME": "hermes", "HERMES_MODE": "external",
             "PROVIDER": "fireworks", "FIREWORKS_API_KEY": "fw-K"},  # no URL
            args=("--advanced",),
        )
        self.assertNotEqual(r.returncode, 0, "external mode without a gateway URL must abort")

    def test_hermes_external_mode_persists_url_no_bridge(self):
        r = self.run_wizard(
            {"SANCHO_RUNTIME": "hermes", "HERMES_MODE": "external",
             "HERMES_GATEWAY_URL": "http://host.docker.internal:8080",
             "PROVIDER": "fireworks", "FIREWORKS_API_KEY": "fw-K"},
            args=("--advanced",),
        )
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(r.env.get("HERMES_GATEWAY_URL"), "http://host.docker.internal:8080")
        self.assertIn(r.env.get("HERMES_BRIDGE_ENABLED", ""), ("", None))

    def test_url_in_env_defaults_to_external_mode(self):
        # A scripted install that pre-sets a gateway URL keeps working (external),
        # so it must NOT silently flip to the bridge and ignore the URL.
        r = self.run_wizard(
            {"SANCHO_RUNTIME": "hermes",
        ...
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(r.env.get("HERMES_GATEWAY_URL"), "https://hermes.example.com")
        self.assertIn(r.env.get("HERMES_BRIDGE_ENABLED", ""), ("", None))

    # --- host ports pruned for non-openclaw (SAN-486) -----------------------

    def test_gateway_host_port_not_persisted_for_hermes(self):
        r = self.run_wizard(
            {"SANCHO_RUNTIME": "hermes", "HERMES_MODE": "bridge",
             "GATEWAY_HOST_PORT": "18999", "LEGACY_HOST_PORT": "18888",
             "PROVIDER": "fireworks", "FIREWORKS_API_KEY": "fw-K"},
            args=("--advanced",),
        )
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertIsNone(r.env.get("GATEWAY_HOST_PORT"))
        self.assertIsNone(r.env.get("LEGACY_HOST_PORT"))

    def test_gateway_host_port_persisted_for_openclaw(self):
        r = self.run_wizard(
            {"SANCHO_RUNTIME": "openclaw", "GATEWAY_HOST_PORT": "18999",
             "PROVIDER": "fireworks", "FIREWORKS_API_KEY": "fw-K"},
            args=("--advanced",),
        )
        self.assertEqual(r.returncode, 0, r.stderr)
        self.assertEqual(r.env.get("GATEWAY_HOST_PORT"), "18999")
