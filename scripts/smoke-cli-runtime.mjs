...
if (!runtimes.has(runtime)) {
  console.error("Usage: node scripts/smoke-cli-runtime.mjs --runtime=claude-code|codex");
  process.exit(1);
}

const runtimeLabel = runtime === "claude-code" ? "Claude Code" : "Codex";
const runtimeAgent = runtime === "claude-code" ? "claude-code" : "codex";
const expectedText =
  process.env.SMOKE_CLI_RUNTIME_EXPECTED ||
  (runtime === "claude-code" ? "sancho-claude-runtime-ok" : "sancho-codex-runtime-ok");
...
function startBridge(port, sanchoPort) {
  const script =
    runtime === "claude-code"
      ? path.join(root, "docker", "runtimes", "claude-code", "bridge.mjs")
      : path.join(root, "docker", "runtimes", "codex", "bridge.mjs");
  const bridgeEnv = {
    ...process.env,
...
async function waitForCompletedRun(runsFile) {
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    latestRun = Array.isArray(runs?.runs)
      ...
      : null;
    if (latestRun?.runtime === "external-http" && latestRun?.status === "completed") {
