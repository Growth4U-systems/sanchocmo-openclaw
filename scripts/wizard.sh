...
if [ "$WIZARD_MODE" = "advanced" ]; then
  ...
  case "$SANCHO_RUNTIME" in
    external-http)
      ...
      ;;
    hermes)
      # Hermes has three shapes; ask which, then collect ONLY that mode's inputs.
      #   bridge   — embedded in the Sancho container (zero external setup); the
      #              gateway URL is derived, the secret autogenerated.
      #   external — point at a gateway you run yourself (BYO URL + secret).
      #   command  — the wizard's HERMES_GATEWAY_COMMAND starts the gateway at boot.
      # Default to bridge, unless a gateway URL is already in the env (scripted /
      # BYO install) — then default to external so that URL isn't silently ignored.
      _hermes_mode_default="bridge"
      [ -n "${HERMES_GATEWAY_URL:-}${HERMES_BASE_URL:-}${HERMES_URL:-}" ] && _hermes_mode_default="external"
      HERMES_MODE="$(ask_choice HERMES_MODE "Hermes mode — bridge (embedded, default), external, or command" "$_hermes_mode_default" bridge external command)"
      case "$HERMES_MODE" in
        bridge)
          # Runs inside the Sancho container. No gateway URL to type — it's the
          # in-container bridge address. Autogenerate the shared secret; the CLI
          # provider is derived from the model provider after that step (below).
          HERMES_BRIDGE_ENABLED=1
          HERMES_BRIDGE_PORT="${HERMES_BRIDGE_PORT:-18795}"
          HERMES_GATEWAY_URL="${HERMES_GATEWAY_URL:-http://127.0.0.1:${HERMES_BRIDGE_PORT}}"
          [ -n "${HERMES_CHAT_SECRET:-}" ] || HERMES_CHAT_SECRET="$(gen_hex)"
          HERMES_CLI="${HERMES_CLI:-hermes}"
          say "  ${DIM}Hermes bridge embedded on :${HERMES_BRIDGE_PORT} (in-container) — no gateway URL needed; secret autogenerated.${RST}"
          ;;
        command)
          HERMES_GATEWAY_COMMAND="$(ask_required HERMES_GATEWAY_COMMAND "Command that starts your Hermes gateway")"
          HERMES_CHAT_SECRET="$(ask HERMES_CHAT_SECRET "Hermes chat secret (blank if none)" "")"
          ;;
        external|*)
          # The URL is called from INSIDE the container — localhost would mean the
          # container itself. A gateway on this host is http://host.docker.internal:<port>.
          say "  ${DIM}The gateway URL is called from inside the Sancho container. For a gateway on THIS host use http://host.docker.internal:<port> — not localhost.${RST}"
          HERMES_GATEWAY_URL="$(ask_required HERMES_GATEWAY_URL "Hermes gateway URL (e.g. http://host.docker.internal:8080)")"
          HERMES_CHAT_SECRET="$(ask HERMES_CHAT_SECRET "Hermes chat secret (blank if none)" "")"
...
if [ "$WIZARD_MODE" = "advanced" ]; then
  ...
  # The OpenClaw gateway (:18789) and the legacy mc-server (:18790) only exist on
  # the openclaw runtime; hermes/external-http never bind them, so don't ask.
...
# Hermes bridge runs the model via its CLI, so derive HERMES_CLI_PROVIDER from the
# model provider chosen above when unset. PROVIDER "both"/"all" isn't a single CLI
# provider — fall back to anthropic (Sancho's primary).
if [ "$SANCHO_RUNTIME" = "hermes" ] && [ "${HERMES_BRIDGE_ENABLED:-0}" = "1" ] && [ -z "${HERMES_CLI_PROVIDER:-}" ]; then
  case "${PROVIDER:-}" in
    anthropic|openai|fireworks) HERMES_CLI_PROVIDER="$PROVIDER" ;;
...
for runtime_key in \
  ...
  HERMES_CLI_PROVIDER HERMES_CLI_MODEL HERMES_CLI_TOOLSETS HERMES_AUTH_SOURCE_DIR \
  HERMES_UNSAFE_ALLOW_DANGEROUS_TOOLSETS HERMES_RUN_TIMEOUT_MS \
  HERMES_CONTEXT_PACK_ENABLED HERMES_CONTEXT_PACK_TIMEOUT_MS \
  SANCHO_CALLBACK_OUTBOX_DIR SANCHO_CALLBACK_TIMEOUT_MS \
  SANCHO_CALLBACK_OUTBOX_MAX_AGE_MS \
...
# Only openclaw uses these host ports — don't persist a stray env value for other
# runtimes (the wizard doesn't ask for them there either).
if [ "$SANCHO_RUNTIME" = "openclaw" ]; then
