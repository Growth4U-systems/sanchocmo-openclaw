...
const knownProject = [
  ...
  "archive_reason", "tool", "completed_at",
];
const knownTask = [
...
async function main() {
const counts = { projects: 0, tasks: 0, contentSubtasks: 0, skippedOrphans: 0 };
      ...
      for (const task of projectTasks) {
        ...
        for (const ct of task.content_tasks || []) {
          ...
          await upsert({
            ...
            brandSlug,
            parentId: childId,
            type: "content_subtask",
            status: String(ct.status || "New"),
            name: String(ct.name || ct.id),
            owner: ct.owner || null,
            skill: ct.skill || null,
            ...
            scheduledFor: toDate(ct.scheduled_for),
