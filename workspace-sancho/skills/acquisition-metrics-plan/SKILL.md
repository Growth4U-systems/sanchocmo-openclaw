...
### Step 7.5: Generate metrics-plan.json (~1 min)

Run the plan generator to create the machine-readable metrics plan that the dashboard reads:
...
This reads `brand/{slug}/integrations.json` and `schemas/integration-mappings.json` to:
1. Map each funnel step to a connected integration (or mark as `manual`)
2. Define KPIs with formulas that reference specific integrations
3. Identify missing integrations the client should connect
4. Generate `brand/{slug}/metrics-plan.json`

The dashboard in Mission Control reads this file to:
- Show the funnel with real numbers from connected APIs
- Calculate cross-source KPIs (CPL = ad spend / new contacts)
- Show which metrics are automated vs manual
- Flag missing integrations with setup links

**If integrations change later** (client connects a new source), re-run this step to update the plan.

---

### Step 8: Integration Discovery Flow

When running Steps 5-5.5 (Map Data Sources + Credential Collection), use the **integration discovery flow**:

For each metric category in the plan, ask the client what tool they use:

> "Para medir **[categoría]** necesito una fuente de datos. ¿Qué usas?"
>
> - **Web traffic**: "¿Tienes Google Analytics configurado?" → Si sí → pedir Property ID → conectar
> - **Leads/CRM**: "¿Dónde guardas tus leads? ¿HubSpot, GoHighLevel, Pipedrive, Sheets?" → conectar la que use
> - **Ads**: "¿Usas ads? ¿Meta, Google, LinkedIn?" → conectar las que use
> - **Social**: "¿Gestionas redes con algún tool? ¿Metricool, Buffer?" → conectar
> - **SEO**: "¿Tienes Search Console verificado?" → conectar
> - **Email/Outreach**: "¿Usas cold email? ¿Instantly, Lemlist?" → conectar

For each tool they mention:
1. Check `schemas/api-catalog.json` for supported integrations
2. If supported → send MC link to connect: `[MC connect link]`
3. If not supported → mark as `manual` and note for future development
4. If they don't have the tool → recommend one based on archetype + budget

After all integrations are collected → run Step 7.5 to generate the plan.

**Available integration mappings** (see `schemas/integration-mappings.json`):
- **Web**: GA4, PostHog
...
### 2. Machine-Readable Plan (metrics-plan.json)

Generated by `generate-plan.js` (Step 7.5). Contains:
- Archetype, activation event, primary KPI
- Funnel steps with source mappings (which API provides each number)
- KPI definitions with formulas (cross-source calculations like CPL)
- Connected integration modules
- Missing integrations the client should connect

This file is read by the Mission Control dashboard to render the metrics page.

Save to: `./brand/metrics-plan.json`

### 3. Excel Tracking Template
...
| WRITE (owns) | `./brand/metrics-plan.md` | Metrics hierarchy + benchmarks + cadence (human-readable) |
| WRITE (owns) | `./brand/metrics-plan.json` | Machine-readable plan for dashboard (generated by generate-plan.js) |
...
| READ | `skills/.../schemas/integration-mappings.json` | How integrations map to metrics |
