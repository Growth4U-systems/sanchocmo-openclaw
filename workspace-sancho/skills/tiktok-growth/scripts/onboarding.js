...
if (validate && configPath) {
  ...
  // Metricool (required)
  if (!config.metricool?.userToken) required.push('metricool.userToken — Metricool user token');
  if (!config.metricool?.userId) required.push('metricool.userId — Metricool user ID');
  if (!config.metricool?.blogId) required.push('metricool.blogId — Metricool blog/brand ID');
  if (!config.metricool?.imageBaseUrl) required.push('metricool.imageBaseUrl — Public URL for images (CDN/S3)');
  ...
  console.log(`   Metricool: ${config.metricool?.blogId ? 'Connected' : 'Not connected'}`);
