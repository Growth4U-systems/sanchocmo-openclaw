...
const ALLOWED_FILES = [
  'legacy-mission-control.html',
  'mc-data.js',
  'legacy-mc-work.js',
  'mc-work.css',
  'legacy-clients.js',
  'legacy-skills-data.js',
  'legacy-agents-data.js',
];
...
<script src="${baseUrl}/legacy-skills-data.js"><\/script>
<style>
...
const mcServer = http.createServer((req, res) => {
  accessLog.write(`${new Date().toISOString()} ${req.method} ${req.url} ua="${(req.headers['user-agent']||'-').replace(/"/g,"'")}" src=${req.socket.remoteAddress||'-'}\n`);
  ...
  if (url.startsWith('/admin/')) {
    ...
    url = rest === '/' ? '/legacy-mission-control.html' : rest;
    ...
    if (url === '/legacy-mission-control.html' || url.endsWith('.html')) {
  ...
  // Block everything else (legacy-mission-control.html, /docs/*, /api/*, /connect/*, /brand/*)
  ...
  if (url.startsWith('/portal/')) {
    ...
    if (portalPath === '/' || portalPath === '') {
      try {
        let html = fs.readFileSync(path.join(BASE, 'legacy-mission-control.html'), 'utf-8');
...
(function() {
  ...
  if (sel) {
    ...
    if (portalPath === '/clients.js') {
      try {
        const allClients = fs.readFileSync(path.join(BASE, 'legacy-clients.js'), 'utf-8');
    ...
    if (portalPath === '/legacy-skills-data.js' || portalPath === '/legacy-agents-data.js') {
