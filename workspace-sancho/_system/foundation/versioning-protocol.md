...
```
brand/{slug}/{pilar}/
  current.md     ← activo (siempre leer este)
  v1.md          ← histórico
  v2.md          ← histórico
...
Cuando un skill referencia `brand/{slug}/market.md` (en context_required o context_writes), lee/escribe `brand/{slug}/market/current.md`.

**Regla**: `brand/{slug}/{nombre}.md` → `brand/{slug}/{nombre}/current.md`
...
1. Comprueba si `{pilar}/current.md` ya existe
2. Si existe → informa: "📄 Ya existe {pilar} (v{N}, {fecha}). ¿Quieres actualizarlo?"
3. Si el usuario confirma → guarda actual como `v{N}.md`, genera nuevo como `current.md`, actualiza `history.json`
