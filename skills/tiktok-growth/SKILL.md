...
## ⚠️ Metricool vs Postiz — Important Differences

This skill is a **fork of Larry**, which was originally built for Postiz. This version uses **Metricool** instead. While most functionality is preserved, there are key differences:

### What Works the Same
- ✅ Per-post analytics (impressions, likes, comments, shares, engagement %)
- ✅ Posting to TikTok (and other platforms)
- ✅ Cross-posting to Instagram, YouTube, Threads, etc.
- ✅ RevenueCat conversion tracking
- ✅ Hook performance tracking and iteration
- ✅ Daily diagnostic reports

### Metricool Limitations
- ⚠️ **No platform-level deltas** — Metricool v2 API only provides per-post data. Cannot track follower growth or total view changes over time.
- ⚠️ **No release ID matching** — Metricool doesn't expose TikTok video IDs, so you can't retroactively connect posts to specific TikTok videos for granular analytics.
- ⚠️ **Only tracks Metricool-created posts** — Posts created directly on TikTok or via other tools won't appear in analytics.
- ⚠️ **Image upload limitation** — Metricool requires images to be publicly accessible URLs. You need to upload slides to a CDN/server before posting (see Image Upload Setup below).

### When to Use This vs Larry
...
- **Use Larry (Postiz)** if: You need follower growth tracking, want automatic post-to-video ID matching, or prefer Postiz's more complete API.

## Image Upload Setup (Metricool Requirement)

Unlike Postiz (which accepts file uploads), Metricool requires images to be **publicly accessible URLs**. Before using this skill, you need to set up image hosting:

### Option 1: CloudFlare R2 (Recommended)
- Free tier: 10GB storage, 10 million reads/month
- S3-compatible, no egress fees
- Set up a public bucket, get the URL
...
- Host images on any web server
- Ensure public read access
- Use the base URL in config
...
"metricool": {
  ...
  "userId": "...",
  "blogId": "...",
  "imageBaseUrl": "https://your-cdn.com/tiktok-posts/"
}
```

The script will construct URLs like `https://your-cdn.com/tiktok-posts/slide1.png`.

**You must upload slide1.png-slide6.png to this URL before calling `post-to-tiktok.js`.**
...
- **Metricool** — this is the backbone of the whole system. Metricool handles posting to TikTok (and 28+ other platforms), but more importantly, it provides the **analytics API** that powers the daily feedback loop. Without Metricool, the agent can post but can't track what's working — and the feedback loop is what makes this skill actually grow your account instead of just posting blindly. Sign up at [metricool.com](https://metricool.com).
...
- **OpenAI** — `gpt-image-1.5` **(ALWAYS 1.5, never 1)**. Needs an OpenAI API key. Best for realistic photo-style images. This is what TikTok Growth uses and what we strongly recommend.
...
> "Do you want to do slideshows (photo carousels) or video? Slideshows are what TikTok Growth uses and what this skill is built around — TikTok's data shows they get 2.9x more comments and 2.6x more shares than video, and they're much easier for AI to generate consistently. That said, if you want to try video, the skill supports it but it hasn't been battle-tested like slideshows have. Your call."
...
> "For the slideshows, we need images. I'd strongly recommend OpenAI's gpt-image-1.5 — it's what TikTok Growth uses and it produces images that genuinely look like someone took them on their phone. It's the difference between 'obviously AI' and 'wait, is that real?' You can also use Stability AI, Replicate, or bring your own images if you prefer."
...
### Phase 4: Metricool Setup (ESSENTIAL — Powers the Entire Feedback Loop)

Metricool isn't just a posting tool — it's what makes the whole feedback loop work. Without it, you're posting blind. With it, you get:
...
This data is what feeds the daily analytics cron (see Phase 8). Without Metricool analytics, the agent can't tell you which hooks are working and which to drop.

Frame it naturally to the user:

> "So here's the key piece — we need Metricool to handle posting and analytics. It's what lets me track every post's performance and tell you exactly which hooks are driving views and which to drop. Without it, we're guessing. With it, I can run a daily report that shows you what's working and automatically suggest better hooks."
>
> "This skill is free and open source. If you want to support its development, signing up through this link is appreciated: [metricool.com](https://metricool.com)"

Walk them through connecting step by step:

1. **Sign up at [metricool.com](https://metricool.com)** — create an account
...
4. **Get the API key** — Settings → API → copy the key. This is how I talk to Metricool programmatically
...
**Don't move on until Metricool is connected and the API key works.** Test it by hitting the platform analytics endpoint. If it returns data, you're good.
...
> "So right now with Metricool, I can track which posts get views, likes, and comments. That's the top of the funnel. But views alone don't pay the bills — we need to know which posts actually drive paying subscribers."
>
> "This is where RevenueCat comes in. It tracks your subscribers, trials, MRR, churn — the actual revenue. When I combine TikTok analytics from Metricool with conversion data from RevenueCat, I can make genuinely intelligent decisions:"
>
...
**Without RevenueCat:** The loop still works on Metricool analytics (views/likes/comments). You can optimize for engagement. But you're flying blind on revenue. You'll know which posts get views but you won't know which posts make money.
...
1. Pulls the last 3 days of post analytics from Metricool
...
```json
{
  ...
  "metricool": {
    "userToken": "your-metricool-user-token",
    "userId": "your-user-id",
    "blogId": "your-blog-id",
    "imageBaseUrl": "https://your-cdn.com/tiktok-posts/",
...
This step uses `node-canvas` to render text directly onto your slide images. This is how TikTok Growth produces slides that have hit **1M+ views on TikTok** — the text sizing, positioning, and styling are dialled in from hundreds of posts.

#### Setting Up node-canvas

Before you can add text overlays, your human needs to install `node-canvas`. Prompt them:

> "To add text overlays to the slides, I need a library called node-canvas. It renders text directly onto images with full control over sizing, positioning, and styling — this is what TikTok Growth uses for his viral TikTok slides.
>
...
#### How TikTok Growth's Text Overlay Process Works
...
**Key details that make TikTok Growth's slides look professional:**
...
**The difference between OK slides and viral slides is in these details.** TikTok Growth's slides consistently hit 50K-150K+ views because the text is sized right, positioned right, and readable at a glance while scrolling.
...
Cross-posts to any connected platforms (Instagram, YouTube, etc.) automatically via Metricool.
...
### 4. Check Post Analytics

**⚠️ METRICOOL LIMITATION:** Unlike Postiz, Metricool does NOT support manual post-to-video ID connection via API. Analytics are automatically tracked for posts created/scheduled through Metricool, but there's no way to retroactively link posts to specific TikTok video IDs.

**What this means:**
- Posts created via Metricool will automatically show up in analytics
- Posts created directly on TikTok won't appear in Metricool analytics
- You cannot manually connect a Metricool post to a specific TikTok video ID
- The daily cron will pull whatever analytics Metricool has available
...
2. Retrieves all posts created via Metricool in the date range
3. Extracts per-post metrics: impressions, likes, comments, shares, engagement %
4. Saves to `analytics-snapshot.json`

**The daily cron handles this automatically.** It runs in the morning, pulls analytics from the last 3 days, and generates the report.

See the Metricool adapter in `skills/metrics-collector/scripts/adapters/metricool.js` for full API implementation.
...
1. **Metricool** → per-post TikTok analytics (views, likes, comments, shares)
...
1. Pulls the last 3 days of posts from Metricool (posts peak at 24-48h)
...
**The daily report automates all of this.** It cross-references TikTok views (Metricool) with downloads and revenue (RevenueCat) and tells you exactly which part of the funnel is broken — per post. It also auto-generates new hook suggestions based on your winning patterns and flags when CTAs need rotating.
...
Metricool supports cross-posting the same content to multiple platforms simultaneously. Recommend:
...
| No cross-posting | Use Metricool to post everywhere simultaneously |
