...
**Auto-arranque del Trust Score (Layer 0, OPCIONAL):** ya NO es trabajo del orquestador — vive en **código** (SAN-309), que es donde es fiable. Cuando el pilar `company-brief` se marca `completed` (ya hay URL), el endpoint `POST /api/brand-brain/pillar-status` dispara `runTrustScore(slug)` (`src/lib/trust-score/run.ts`) **fire-and-forget**, una sola vez (guard: solo si aún no hay cache `metrics/trust-score.json`). Corre el analyzer en modo compare (auto-descubre + fija competidores), persiste la métrica diaria (`sources.trust_score`), escribe `site-audit/trust-score/trust-score.current.md` y enciende la superficie Reputation. NO bloquea: si el analyzer falla (sin competidores → 409, discovery caído → 502, o `_stale`), el pilar `trust-score` queda pendiente **sin trabar** el avance ni el gate de Foundation. `trust-score` nunca es `requires` de ningún pilar downstream; el refresco recurrente lo hace el cron `trust_score_refresh`.
