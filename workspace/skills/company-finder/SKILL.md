---
name: company-finder
description: Encuentra empresas que matchean tu ICP usando Apollo.io, Clay, o LinkedIn Sales Navigator. Lee ICP de niche-discovery-100x, mapea criterios a filtros de busqueda (industria, tamano, geografia, tech stack, funding), ejecuta busqueda, enriquece firmographics, aplica ICP fit score (1-10), filtra score>=7, prioriza por signals (hot>warm>cold). Output companies-YYYYMMDD.json. Fase Encuentra SanchoCMO (one-to-one outreach). Use cuando necesitas lista de empresas target, tienes ICP definido, o usuario dice find companies, busca empresas, prospect list, build company list, ICP search, find leads. Do NOT use para encontrar personas dentro de empresas (usar decision-maker-finder) ni para enriquecer contactos (usar contact-enrichment).
metadata:
