...
description: >
  Classifies a client's business model (B2B/B2C, revenue model, growth motion) and
  maps their current acquisition funnel. Foundation pillar (Layer 1) — depends on
  company-context. Use when onboarding a new client, when foundation-orchestrator
  routes here, when user says "business model", "revenue model", "how we make money",
  "PLG vs MLG", "acquisition funnel", or "growth model". Also triggers when
  business-model-audit status is empty in the Context Lake. Do NOT use for pricing
  strategy (use pricing-hooks), financial projections (out of scope), or detailed
  unit economics (handled in phase-0-diagnostic scoring).
metadata:
