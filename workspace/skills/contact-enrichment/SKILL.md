...
description: Enriquece contactos de decision-maker-finder con emails verificados, telefonos directos, y perfiles sociales usando waterfall enrichment (Apollo->Hunter.io->SignalHire->Snov.io). Verifica emails (MX records, SMTP, deliverability score), encuentra phone numbers (mobile + direct dial), agrega social profiles. Output contacts-enriched-YYYYMMDD.json listo para outreach. PASO FINAL antes de outreach. Fase Encuentra SanchoCMO (one-to-one). Use cuando tienes decision makers y necesitas datos de contacto, despues de decision-maker-finder, o usuario dice enrich contacts, get emails, find email for Person, enriquecer contactos, get phone numbers, waterfall enrichment. Do NOT use para encontrar empresas (usar company-finder) ni para encontrar personas (usar decision-maker-finder). Esta skill solo ENRIQUECE contactos ya identificados.
metadata:
