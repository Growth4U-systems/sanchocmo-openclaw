...
description: "Researches competitors using the 3-lens intelligence methodology (Autopercepcion, Terceros, Consumidor) — same pipeline as self-intelligence but pointed outward. Produces a Battle Card per competitor and Competitive Landscape Map. Foundation pillar (Layer 2), depends on company-context. Three entry modes: user tells you competitors, Sancho researches independently, or user gives partial guidance. Use when foundation-orchestrator routes here, when user says competitor analysis, competitive research, who are our competitors, battle card, competitive landscape, spy on competitors, or competitor comparison. Also triggers when competitor-intelligence status is empty in Context Lake. Do NOT use for our own brand analysis (use self-intelligence), competitor SEO pages (use competitor-alternatives), or per-niche competitor deep-dive (that's part of positioning-messaging Step 2)."
metadata:
