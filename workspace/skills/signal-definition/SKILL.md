...
description: Definir qué señales de compra importan para un cliente específico basándose en ICP, ECPs, y sector. Analiza contexto del cliente, propone señales relevantes (funding, hiring, tech stack, intent, complaints), aplica scoring (urgency × qualification), genera signals-to-track.json para signal-monitor. STEP 0 del Signal Detection workflow - ejecutar ANTES de signal-monitor. Fase Encuentra SanchoCMO (one-to-one). Use cuando empiezas outreach strategy, tienes ECPs definidos, o usuario dice define signals, qué señales buscar, signal detection setup, buying signals for my ICP, what triggers should I monitor. Do NOT use para monitorear señales (usar signal-monitor). Esta skill define QUÉ buscar, signal-monitor ejecuta.
metadata:
