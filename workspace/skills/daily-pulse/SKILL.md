---
name: daily-pulse
description: Extrae insights automaticos de comunicaciones diarias (Slack channels, Notion meeting notes, transcripts) y genera ideas de contenido clasificadas. Escanea mensajes recientes, extrae pain points, feature requests, success stories, preguntas frecuentes, trends, clasifica cada insight, y genera micro-briefs con titulo, angulo, y tipo de contenido. Output es daily-pulse-YYYYMMDD.json. Fase Encuentra SanchoCMO (one-to-many, intelligence loop). Use cuando quieras extraer contenido de conversaciones, hacer daily content mining, o usuario dice daily pulse, extract insights from slack, what are we talking about, content ideas from meetings, que estan diciendo los clientes. Do NOT use para crear contenido final (usar seo-content o content-atomizer). Esta skill EXTRAE ideas, no las ejecuta.
metadata:
