...
description: >
  Captures a client's marketing budget, timeline, team capacity, and existing tool
  stack. Always-first pillar (Layer 0) — no dependencies. Use when onboarding a new
  client, when foundation-orchestrator routes here, when user says "budget", "what
  can we spend", "our resources", "tools we use", or "team capacity". Also triggers
  when budget-constraints status is empty in the Context Lake. Do NOT use for pricing
  strategy (use pricing-hooks), channel-specific budget splits (handled in Phase 3),
  or financial modeling (out of scope).
metadata:
