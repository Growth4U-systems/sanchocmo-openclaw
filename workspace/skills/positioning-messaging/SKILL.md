...
description: "Builds per-niche positioning and messaging using a 7-step pipeline: niche deep research, mini competitor analysis, own-company analysis, value criteria discovery with competitive scoring, asset mapping (Qualifiers vs Differentiators), benefit-proof pairing, and bilingual messaging playbook. Foundation pillar (Layer 4) — depends on niche-discovery-100x and competitor-intelligence. Manages Tier 2 shared documents (value criteria, assets) across niches. Use when foundation-orchestrator routes here, when user says positioning, messaging framework, value proposition, how to differentiate, what should we say to [niche], per-niche messaging, or UVP/USP. Also triggers when positioning status is empty in the Context Lake. Do NOT use for brand voice/tone (use brand-voice), ad copywriting (use paid-ads or social-content), or pricing strategy (use pricing-hooks)."
metadata:
