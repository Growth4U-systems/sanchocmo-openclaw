...
description: "Create personalized OUTBOUND cold sequences per ECP. Multi-channel (email + LinkedIn + optional video), 5-7 touchpoints with timing, signal-based triggers from signal-definition output. DIFFERENT from email-sequences which is INBOUND post-opt-in nurture. This is COLD outreach: personalized, multi-touch, GDPR compliant. Supports Tool Detection: FULL (Instantly/Lemlist/Expandi) vs STANDARD (email client) vs LIGHT (manual). Reads channel-plan, positioning, ecps, voice-profile, contacts-enriched.json, signals-to-track.json. Writes sequence files to ./campaigns/outreach-{ecp}/. Phase Decide SanchoCMO (one-to-one). Use when contacts are enriched and client needs cold outreach sequences, or user says create outreach, cold email, LinkedIn outreach, prospecting cadence, outbound sequence. Do NOT use for warm email nurture (use email-sequences) or finding contacts (use contact-enrichment)."
metadata:
