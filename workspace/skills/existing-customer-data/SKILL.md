...
description: >
  OPTIONAL pillar. Analyze existing customer data from CRM for segmentation, churn patterns,
  and best customer profile identification. Use when client has customers (>50) and CRM access
  available. Skip if pre-launch or no CRM. Uses RFM analysis (Recency, Frequency, Monetary),
  behavioral clustering, LTV analysis, and churn pattern detection. Outputs customer segments,
  best customer profile, upgrade patterns, and churn insights to ./brand/customer-data.md.
  Triggers: "analyze customers", "CRM segmentation", "customer data", "churn analysis".
metadata:
