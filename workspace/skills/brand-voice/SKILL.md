---
name: brand-voice
description: "Defines or extracts brand voice: how the brand speaks, what words to use/avoid, Do This/Not That examples, and an AI Brand Kit for consistent content generation. Two modes — Quick (30 min, from URL or 5 questions) and Full (2-3 hours, per-audience tone adaptation + per-channel guidance). Use when user says brand voice, tone of voice, how should we sound, voice guide, brand personality, what words should we use, or when brand-voice status is empty. Do NOT use for visual identity (use visual-identity), ad copywriting (use direct-response-copy), or messaging strategy (use positioning-messaging)."
metadata:
