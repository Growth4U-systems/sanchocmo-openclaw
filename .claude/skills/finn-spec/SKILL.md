---
name: finn-spec
description: Interview the user about a raw idea until confident, then file a build-ready SanchoCMO Linear issue (SAN-<n>). Use when asked to run the loop's spec interview, draft a queue-ready issue, or plan a feature. Interactive — requires the user present; never run unattended.
---
...
> **Repo conventions.** This is the `spec` stage of the assisted loop (setup and
> requirements: `.claude/skills/finn-loop-setup.md`). Branch/commit/PR rules live
> in `docs/CONTRIBUTING.md` and the `git-workflow` skill — this skill only files
> the issue; it does not branch or code. The Linear team is **SanchoCMO** (issue
> ids `SAN-<n>`).
...
- Size the issue to one day of agent work or less. Bigger work becomes a
  ...
  code from the ones before it. Use Linear blocked-by relations for the chain.

## 4. Confirm and file

Show the full draft in chat and get the user's go-ahead. Then create the
issue on the **SanchoCMO** Linear team (via the Linear connector) with the
draft as the body. Report the exact issue identifier (`SAN-<n>`) and URL
returned by Linear; later skills use that identifier rather than guessing it.
