...
CREATE TABLE IF NOT EXISTS "agent_run_events" (
	...
	CONSTRAINT "agent_run_events_type_check"
		CHECK ("type" IN (
			'run_created', 'runtime_dispatched', 'runtime_rejected',
			'runtime_unreachable', 'progress', 'artifact_readback', 'bot_reply',
			'cancel_requested', 'cancel_dispatched', 'cancel_failed', 'failed'
		))
);
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_runs_thread_status_idx" ON "agent_runs" ("thread_id", "status", "created_at");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_run_events_run_sequence_idx" ON "agent_run_events" ("run_id", "sequence");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_run_events_thread_sequence_idx" ON "agent_run_events" ("thread_id", "sequence");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_run_events_trace_idx" ON "agent_run_events" ("trace_id");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "agent_run_events_ts_idx" ON "agent_run_events" ("ts");
