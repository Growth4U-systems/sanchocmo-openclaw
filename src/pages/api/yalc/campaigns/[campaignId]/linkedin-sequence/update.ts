...
import { assertCampaignKind, expectedCampaignKindFromInput, yalcGuardErrorResponse } from "@/lib/yalc/campaign-guards";
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    await assertCampaignKind(config, campaignId, expectedCampaignKindFromInput(body));
    return res.status(200).json(
      await yalcFetch(
        config,
        `/api/campaigns/${encodeURIComponent(campaignId)}/linkedin-sequence/update`,
        { method: "POST", body },
      ),
    );
  } catch (err) {
