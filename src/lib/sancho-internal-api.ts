...
import { getLatestMetricSummary } from "@/lib/data/metrics";
...
export async function getInternalClientStatus(
  slug: string
): Promise<SanchoClientLiveStatus | null> {
  ...
  return {
    ...
    metrics: await getClientMetricsSummary(slug),
  };
}

/**
 * Reads the latest metric rows from metric_snapshots and returns a compact
 * per-source summary so the client status/report carries its KPIs. Returns null
 * if the DB is unconfigured or no metrics have been collected yet.
 */
export async function getClientMetricsSummary(slug: string): Promise<SanchoMetricSummary | null> {
  const summary = await getLatestMetricSummary(slug);
  if (!summary.configured || summary.sources.length === 0) return null;
  return {
    collected_at: summary.collectedAt,
