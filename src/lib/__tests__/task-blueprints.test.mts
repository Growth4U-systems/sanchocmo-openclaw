...
test("instantiateTaskSet('content') reproduces the legacy create-project spec (+ agent)", () => {
  ...
  const expected = [
    ...
    },
    {
      ...
      description: `Rellena los configs existentes (news-prompts, paa-queries, keywords-seed, sources.json profiles, cadence-config.yml) con datos derivados de content-pillars.md + pov-bank.json + el keyword-plan validado (keyword-clusters.json, T07) + Foundation. keywords-seed deriva de los clusters validados de T07, no de seeds genericos. Genera ademas un setup.md narrativo que explica el por que de cada decision y enlaza con los crones que consumen cada config. La infraestructura (carpetas + YAMLs + crons) ya existe — esta tarea solo MODIFICA los campos editables y DOCUMENTA. Las voces founder-led (persona + red + handle + cuenta de publicacion) se delegan en la sub-skill founder-led-setup, que escribe los profiles[] por voz bajo cada canal en cadence-config.yml. ORDEN DE EJECUCION: SE EJECUTA EL ULTIMO. Requiere ${projectId}-T01 (Strategy) + ${projectId}-T02 (Pillars) + ${projectId}-T04 (POV Bank) + ${projectId}-T07 (Keyword Research) en status:completed.`,
      phase: 1,
      type: "execution",
      channel: "strategy",
      niche: null,
    ...
    {
      id: "P07-T07",
      name: "Keyword Research & SERP Clustering",
      description: "Ejecutar keyword-research: valida los spokes de content-pillars.md contra datos de búsqueda reales (6 Circles + SERP-overlap clustering + 4-test pillar validation) y escribe keyword-plan.md (lo lee el writer seo-content) + keyword-clusters.json. ORDEN: corre tras T02 (Pillars); T03 (Setup) lo consume para derivar keywords-seed del plan validado.",
      phase: 1,
      type: "foundation",
      channel: "strategy",
      niche: null,
      status: "todo",
      deliverable: "Keyword plan validado (clusters por SERP-overlap + 4-test pillars) que alimenta keywords-seed y el writer",
      deliverable_file: `brand/${slug}/go-to-market/keyword-plan.md`,
      output_files: ["keyword-plan.md", "keyword-clusters.json"],
      depends_on: "P07-T02",
      owner: "Sancho",
      skill: "keyword-research",
      agent: "dulcinea",
      mc_chat_thread_id: tid("t07"),
    },
  ];
