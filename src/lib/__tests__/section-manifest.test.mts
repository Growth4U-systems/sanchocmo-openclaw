...
// ── F2 equivalence: derived Content Engine tasks === the frozen legacy spec ─
// The previous hardcoded array in create-project.ts is the spec. The manifest-
// derived builder must reproduce it byte-for-byte, PLUS the newly co-located
// `agent` field. Guards the create-project migration against any drift.
const { buildContentEngineTasks } = await import("../data/content-engine-tasks");

test("buildContentEngineTasks reproduces the legacy create-project spec (+ agent)", () => {
  const slug = "growth4u";
  const projectId = "P07";
  ...
  assert.deepEqual(buildContentEngineTasks(slug, projectId), expected);
});
